...
setInterval(()=> {
  try{
    ...
    cleanExpiredLocks();
  } catch(err){ console.error("Error in flush:",err); }
},100);
...
function handleJoinRoom(ws:WebSocketWithRoom,newRoom:RoomName) {
  ...
  const allPoints:any[]=[]; const meta:Record<number,Omit<SeatInfo,"points">>={};
  for (const [seat,info] of seatMap) {
    for (const p of info.points) allPoints.push({seat,...p});
    if (info.namauser&&!info.namauser.startsWith("__LOCK__")) {
      const {points,...rest}=info; meta[seat]=rest;
    }
  }
...
// ===== Dispatcher (string persis dengan kode awal) =====
...
;(async()=>{
  for await(const events of kv.watch([["room"],["private"],["notif"]])) {
    for (const e of events) {
      const key=e.key as (string|number)[]; const val=e.value as any;
