...
function handleGetAllRoomsUserCount(ws: WebSocketWithRoom) {
  ...
  safeSend(ws, ["allRoomsUserCount", roomList.map(r => [r, allCounts[r]])]);
}
...
// ===== Event Handlers (ringkas) =====
function handleSetIdTarget(ws: WebSocketWithRoom, id: string) {
  ws.idtarget = id; safeSend(ws, ["setIdTargetAck", ws.idtarget]);
}
...
function handleMessage(ws: WebSocketWithRoom, dataStr: string) {
  try {
    ...
    switch (evt) {
      ...
      case "chat": if (!chatMessageBuffer.has(args[0])) chatMessageBuffer.set(args[0], []); chatMessageBuffer.get(args[0])!.push(["chat", ...args]); break;
...
// (implementasi handleUpdatePoint, handleRemoveKursi, handleUpdateKursi, handleSendNotif, handlePrivate, handleIsUserOnline sama dengan sebelumnya)

// ===== Serve WebSocket + Health Check =====
serve((req) => {
  const { pathname } = new URL(req.url);
  // ✅ Health check untuk Deno Deploy
  if (pathname === "/") {
  ...
  try {
    ...
    ws.onclose = () => { clients.delete(ws); ws.numkursi?.clear(); ws.roomname = undefined; };
    return response;
  } catch {
